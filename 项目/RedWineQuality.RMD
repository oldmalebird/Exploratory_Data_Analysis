...
### 你的数据集结构是什么？
数据集包含1599个观察值，13个变量。其中，变量x起索引作用，其余变量除品质分数为整型外，均为浮点型。
...
ggplot(data=redwine, aes(x = density, y = alcohol)) +
  ...
  ylim(5, quantile(redwine$alcohol, 0.95)) +
...
柠檬酸带来的风味可以给红葡萄酒加分，挥发性酸中的乙酸会带来刺鼻的醋味给葡萄酒减分。而图中显示，柠檬酸和挥发性酸呈现负相关，是不是存在一种影响因素可以在降低挥发性酸的同时增加柠檬酸？

# 反思
红葡萄酒品质数据集中缺乏像钻石数据集中的多个因子变量，因此不能以预想的方式从多个维度来进行分析，更多只能进行单、双变量分析。 

在一步步的分析中，破除了我对总二氧化硫与硫酸盐的关系、柠檬酸与挥发性酸的关系的猜想，说明分析问题不能只靠直觉，而更应该靠可靠的数据分析。 
